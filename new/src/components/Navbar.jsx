        ...
        <div className="relative flex gap-4 sm:gap-6 md:gap-8 whitespace-nowrap">
          {navLinks.map((link, index) => (
            <a
              ...
              className="relative px-2 py-1 text-xs sm:text-sm md:text-base text-gray-300 hover:text-white font-medium"
            >
              {activeIndex === index && (
                <motion.span
                  layoutId="nav-arc"
                  className="absolute -top-3 left-1/2 -translate-x-1/2 h-4 w-12 rounded-b-full bg-purple-500/40 blur-md"
                  transition={{
                    type: "spring",
                    stiffness: 500,
                    damping: 35,
